...
final class AssetDetailViewController: KLModuleViewController, KLVMVC {
    ...
    func config(constructor: AssetDetailViewController.Config) {
        ...
        tabVC = TransRecordListViewController.instance(from: TransRecordListViewController.Config(
            asset: constructor.asset, records: [], type: .total
            ))
...
//    @IBOutlet weak var withdrawalBtn: UIButton!
    ...
    private var tabVC: TransRecordListViewController!
...
//        renderNavBar(tint: palette.application_main, barTint: palette.nav_bg_clear)
//        renderNavTitle(color: palette.application_main, font: .owMedium(size: 18))

//        changeNavShadowVisibility(false)
        
//        changeLeftBarButtonToDismissToRoot(tintColor: palette.application_main, image: #imageLiteral(resourceName: "arrowNavBlack"), title: nil)
...
//        assetAmtLabel.set(textColor: palette.application_main, font: .owMedium(size: 26))
...
//        let coin = viewModel.input.asset.coin!
        self.titleLabel.text = viewModel.input.asset.coin?.inAppName
