//
//  WithdrawalCICFeeInfoQuickModeViewModel.swift
//  OfflineWallet
//
...
class WithdrawalCICFeeInfoQuickModeViewModel: KLRxViewModel, WithdrawalETFFeeInfoModeBase {
    struct Input {
    ...
    var input: WithdrawalCICFeeInfoQuickModeViewModel.Input
    var output: Void
    ...
    public var gasInfo: Observable<(gasPrice: Decimal?, gas: Decimal?)> {
        return Observable
            .combineLatest(
                gasPrice, Observable.just(FeeManager.getValue(fromOption: .cic(.gas(mainCoinID: input.mainCoinID))))
            )
            .map {
                (gasPrice: $0, gas: $1)
        }
