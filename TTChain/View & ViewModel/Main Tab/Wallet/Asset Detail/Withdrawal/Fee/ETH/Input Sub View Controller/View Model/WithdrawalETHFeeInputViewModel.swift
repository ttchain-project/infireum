//
//  WithdrawalETHFeeInputViewModel.swift
//  OfflineWallet
//
...
class WithdrawalETHFeeInputViewModel: KLRxViewModel, WithdrawalFeeInfoProvider {
...
//    }()
    ...
    private lazy var _fiatRate: BehaviorRelay<Decimal?> = {
        let dbRate = CoinToFiatRate.getRateFromDatabase(coinID: Coin.eth_identifier, fiatID: input.fiat.id)?.rate
        let relay = BehaviorRelay<Decimal?>.init(value: dbRate as Decimal?)
        
        CoinToFiatRate.getRateFromServerIfPossible(coin: Coin.eth, fiat: input.fiat)
            .asObservable()
            .bind(to: relay)
            .disposed(by: bag)
