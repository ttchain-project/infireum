//
//  WithdrawalLightningPwdValidationViewController.swift
//  OfflineWallet
//
...
final class WithdrawalLightningPwdValidationViewController: KLModuleViewController, KLVMVC {
    ...
    typealias ViewModel = WithdrawalLightningPwdValidationViewModel
    var viewModel: WithdrawalLightningPwdValidationViewModel!
    var bag: DisposeBag = DisposeBag.init()
    
    func config(constructor: WithdrawalLightningPwdValidationViewController.Config) {
        view.layoutIfNeeded()
        viewModel = ViewModel.init(
            input: WithdrawalLightningPwdValidationViewModel.InputSource(
    ...
    private func bindViewModel() {
        ...
        viewModel.isPwdVisible.map { !$0 }.bind(to: pwdTextField.rx.isSecured).disposed(by: bag)
    ...
    override func renderLang(_ lang: Lang) {
        let dls = lang.dls
        title = dls.ltTx_pwdVerify_title
        titleLabel.text = dls.ltTx_pwdVerify_label_input_wallet_pwd
        pwdTextField.set(placeholder: dls.ltTx_pwdVerify_placeholder_input_wallet_pwd)
    ...
    private func showInvalidPwdAlert() {
        let dls = LM.dls
        showSimplePopUp(with: dls.ltTx_pwdVerify_error_pwd_is_wrong,
                        contents: "",
    ...
    private func handleTransferState(_ state: LightningTransferFlowState) {
        let dls = LM.dls
        switch state {
            ...
            case .success(let record):
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                    OWRxNotificationCenter.instance
                        .lightningTransferRecordCreated(record)
